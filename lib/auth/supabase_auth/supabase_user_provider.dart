...
class ProyectoSupabaseUser extends BaseAuthUser {
  ProyectoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> proyectoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProyectoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
